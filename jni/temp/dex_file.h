...
class DexFile {
 public:
  ...
  void PrintInfoWithValue(std::string info, uint64_t value) const;
  void PrintMemberOffset() const;
  ...
  const StringId& GetStringId(dex::StringIndex idx) const {
    // PrintInfo("[Moran]: <GetStringId> Enter, string_ids_: ");
    DCHECK_LT(idx.index_, NumStringIds()) << GetLocation();
    // PrintInfo("[Moran]: <GetStringId> DCHECK pass");
    // PrintInfoWithValue("[Moran]: <GetStringId> idx.index_: ", (uint64_t)idx.index_);
    // PrintInfoWithValue("[Moran]: <GetStringId> Enter, string_id.string_data_off_: ", (uint64_t)string_ids_[idx.index_].string_data_off_);
    // LOG(INFO) << "[Moran]: <GetStringId> string_ids_:" << string_ids_;
    //  LOG(INFO) << "[Moran]: <GetStringId> idx.index_:" << idx.index_;
    //   LOG(INFO) << "[Moran]: <GetStringId> string_id.string_data_off_:" << string_ids_[idx.index_].string_data_off_;
    return string_ids_[idx.index_];
  }

//   const StringId* GetBaseStringId() const{
//       return string_ids_;
//   }
  ...
  const TypeId& GetTypeId(dex::TypeIndex idx) const {
    // PrintInfo("[Moran]: <GetTypeId> Enter");
    DCHECK_LT(idx.index_, NumTypeIds()) << GetLocation();
    // PrintInfo("[Moran]: <GetTypeId> DCHECK pass");
    // LOG(INFO) << "[Moran]: <GetTypeId> type_ids_ pointer is: " << type_ids_;
    // LOG(INFO) << "[Moran]: <GetTypeId> idx_index_ is: " << idx.index_;
    return type_ids_[idx.index_];
  }

  

//   const TypeId& MoranGetTypeId(dex::TypeIndex idx) const {
//     // PrintInfo("[Moran]: <GetTypeId> Enter, type_ids_ is ");
//     DCHECK_LT(idx.index_, NumTypeIds()) << GetLocation();
//     // PrintInfo("[Moran]: <GetTypeId> DCHECK pass, idx.index_ is ");
//     // LOG(INFO) << "[Moran]: <GetTypeId> type_ids_ pointer is: " << type_ids_;
//     // LOG(INFO) << "[Moran]: <GetTypeId> idx_index_ is: " << idx.index_;
//     return type_ids_[idx.index_];
//   }
  ...
  const char* MoranStringByTypeIdx(dex::TypeIndex idx) const;
  ...
  const ClassDef& GetClassDef(uint16_t idx) const {
    ...
    // if (rand() % 100 == 1){
    //     PrintInfo("[Moran] <GetClassDef> DCHECK pass");
    // }
    return class_defs_[idx];
  }

  void CheckAllMemberPointer() const {
      if (class_defs_ == nullptr){
          PrintInfo("class_defs_ is null");
      }
      else{
          PrintInfoWithValue("class_defs_ is ", (uint64_t)class_defs_);
      }

      if (header_ == nullptr){
          PrintInfo("header_ is null");
      }
      else{
          PrintInfoWithValue("header_ is ", (uint64_t)header_);
      }

      if (string_ids_ == nullptr){
          PrintInfo("string_ids_ is null");
      }
      else{
          PrintInfoWithValue("string_ids_ is ", (uint64_t)string_ids_);
      }
      if (type_ids_ == nullptr){
          PrintInfo("type_ids_ is null");
      }
      else{
          PrintInfoWithValue("type_ids_ is ", (uint64_t)type_ids_);
      }
      if (field_ids_ == nullptr){
          PrintInfo("field_ids_ is null");
      }
      else{
          PrintInfoWithValue("field_ids_ is ", (uint64_t)field_ids_);
      }
      if (method_ids_ == nullptr){
          PrintInfo("method_ids_ is null");
      }
      else{
          PrintInfoWithValue("method_ids_ is ", (uint64_t)method_ids_);
      }
      if (proto_ids_ == nullptr){
          PrintInfo("proto_ids_ is null");
      }
      else{
          PrintInfoWithValue("proto_ids_ is ", (uint64_t)proto_ids_);
      }
      
      PrintInfoWithValue("kDexNoIndex16 is ", (uint64_t)kDexNoIndex16);

      if (begin_ == nullptr){
          PrintInfo("begin_ is null");
      }
      else{
          PrintInfoWithValue("begin is ", (uint64_t)begin_);
      }

      // For size_
      PrintInfoWithValue("size is ", (uint64_t)size_);

      // For location
      PrintInfo("location_ is not null, location is ");
      PrintInfo(location_);
      
      // Test whether class_defs_ points to null
      PrintInfo("Begin to test the result of class_defs_[0]");
      const ClassDef& class_def = class_defs_[0];
      PrintInfo("class_defs_ has been called");
      PrintInfoWithValue("idx.index_ is:", (uint64_t)class_def.class_idx_.index_);
  ...
  const char* MoranGetClassDescriptor(const ClassDef& class_def) const;
